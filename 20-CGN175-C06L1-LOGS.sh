...
# download archive from github; copy and load/merge on bigip; also make other config changes
curl --silent https://raw.githubusercontent.com/learnf5/$COURSE_ID/main/3_cgnat_log.scf -o /tmp/3_cgnat_log.scf
sudo scp /tmp/3_cgnat_log.scf 192.168.1.31:/var/local/scf
...
sudo ssh 192.168.1.31 tmsh load sys config merge file 3_cgnat_log.scf
